functional.betapart.core<-function (x, traits, multi = TRUE, warning.time = TRUE, return.details = FALSE,prefix="") 
{
  if (!is.matrix(x)) {
    x <- as.matrix(x)
  }
  ...
  for (i in 1:N) {
    ...
    vert0 <- convhulln(tr_i, paste0("Fx TO"," '",paste0(prefix,"vert.txt"),"'"))
    ...
    step.fbc[, 1] <- as.character(step.fbc[, 1])
    write.table(step.fbc, file = paste0(prefix,"step.fbc.txt"), row.names = T, 
                col.names = F, sep = "\t")
  }
  sumFRi <- sum(FRi)
  ...
  if (N > 2 & multi == T) {
    ...
    for (z in 3:N) {
      ...
      for (k in 1:ncol(comb_z)) {
        ...
        write.table(step.fbc, file = paste0(prefix,"step.fbc.txt"), 
...
functional.beta.pair<-function (x, traits, index.family = "sorensen",prefix="") 
{
  index.family <- match.arg(index.family, c("jaccard", "sorensen"))
  fbc <- x
  if (!inherits(x, "functional.betapart")) {
    fbc <- functional.betapart.core(x, traits, multi = FALSE, 
                                    warning.time = FALSE, return.details = FALSE,prefix = prefix)
  }
